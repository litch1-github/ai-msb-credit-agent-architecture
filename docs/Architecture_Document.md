...
| Код | Объект данных                  | Источник                                                       | Получатель                                                     | Сущности в потоке                                                                                                 | Описание                                                                                                           | Тип      | Протокол           | Нагрузка                      |
| --- | ------------------------------ | -------------------------------------------------------------- | -------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | -------- | ------------------ | ----------------------------- |
| П01 | Заявка на кредит               | Кредитный менеджер                                             | Слой: Интерфейс (IDP UI)                                       | JSON: анкета клиента + пакет документов (PDF/DOCX, до 200 МБ)                                                     | Новый поток. Менеджер загружает анкету и пакет документов через IDP UI и нажимает «Запустить обработку».           | Request  | HTTPS              | ~5 МБ на заявку               |
| П02 | Запрос аутентификации          | Слой: Интерфейс (IDP UI)                                       | Слой: Безопасность (Keycloak, Vault, Policy Hook Bus)          | {user_id, mfa_challenge, session_id}                                                                              | Новый поток. IDP Portal инициирует аутентификацию в корпоративном IdP (Keycloak).                                  | Request  | OIDC/OAuth2        | ~1 КБ                         |
| П03 | JWT-токен с правами            | Слой: Безопасность (Keycloak, Vault, Policy Hook Bus)          | Слой: Интерфейс (IDP UI)                                       | {access_token, roles, r_level, permissions}                                                                       | Новый поток. Keycloak возвращает JWT с ролями и вычисленным R-level для доступа к flows.                           | Response | OIDC               | ~2 КБ                         |
| П04 | Заявка на обработку            | Слой: Интерфейс (IDP UI)                                       | Слой: Оркестратор (Temporal, A2A/Kafka)                        | A2A message: {task_id, spec_pointer: "SDD-10293", context_bundle, user_jwt, constraints}                          | Новый поток. IDP Portal валидирует заявку, формирует A2A-сообщение и передаёт в Orchestrator через Temporal/Kafka. | Request  | A2A (JSON-RPC)     | ~20 КБ                        |
| П05 | Декомпозиция задач             | Слой: Оркестратор (Temporal, A2A/Kafka)                        | Слой: Агенты (Review, Scoring, Document, Guardian, Compliance) | A2A: {task_id, subtasks: \[...\], spec_pointer, sandbox_profile}                                                  | Новый поток. Orchestrator декомпозирует заявку на подзадачи и маршрутизирует специализированным агентам.           | Request  | A2A (JSON-RPC)     | ~10 КБ                        |
| П06 | Запрос политик                 | Слой: Агенты (Review, Scoring, Document, Guardian, Compliance) | Слой: Безопасность (Keycloak, Vault, Policy Hook Bus)          | {action: "pdn_process", context: {...}, policy_ids: \["152-FZ-v2.1", "CBR-AML-v1.0"\]}                            | Новый поток. Агенты запрашивают политики безопасности и compliance-правила перед выполнением действий.             | Request  | gRPC + mTLS        | ~5 КБ                         |
| П07 | Политики и разрешения          | Слой: Безопасность (Keycloak, Vault, Policy Hook Bus)          | Слой: Агенты (Review, Scoring, Document, Guardian, Compliance) | {decision: ALLOW/DENY/MODIFY/ESCALATE, policies: \[...\], r_level: 2}                                             | Новый поток. Policy Hook Bus возвращает решение по политикам (OPA Rego) и активные ограничения.                    | Response | gRPC + mTLS        | ~5 КБ                         |
| П08 | Запрос контекста               | Слой: Агенты (Review, Scoring, Document, Guardian, Compliance) | Слой: Данные (RAG/Cache/Evidence/Audit)                        | {query_vector, collection: "credit_policies", top_k: 5}                                                           | Новый поток. Агенты запрашивают корпоративные знания и контекст из RAG/Cache для принятия решений.                 | Request  | HTTPS              | ~5 КБ                         |
| П09 | Корпоративные знания           | Слой: Данные (RAG/Cache/Evidence/Audit)                        | Слой: Агенты (Review, Scoring, Document, Guardian, Compliance) | {context_documents: \[...\], policies: \[...\], patterns: \[...\]}                                                | Новый поток. RAG возвращает релевантные документы, политики кредитования и паттерны обработки.                     | Response | HTTPS              | ~50 КБ                        |
| П10 | Запрос к системам              | Слой: Агенты (Review, Scoring, Document, Guardian, Compliance) | Слой: Интеграция (AI Gateway / MCP)                            | MCP request: {tool_id: "ocr_service.extract", params: {file_uri}}                                                 | Новый поток. Агенты запрашивают доступ к корпоративным системам через MCP Proxy.                                   | Request  | MCP (HTTPS)        | ~10 КБ                        |
| П11 | Запрос к АБС/распознаванию     | Слой: Интеграция (AI Gateway / MCP)                            | Слой: Корпоративные системы (Распознавание, АБС)               | HTTPS POST: {subject_id, consent_token, query_type: "credit_history"}                                             | Новый поток. MCP Proxy проксирует запросы к АБС (банковское ядро) и сервису распознавания документов.              | Request  | HTTPS + mTLS + JWT | ~50 КБ                        |
| П12 | Данные от корпоративных систем | Слой: Корпоративные системы (Распознавание, АБС)               | Слой: Интеграция (AI Gateway / MCP)                            | {credit_history: {...}, account_data: {...}, recognized_text: "..."}                                              | Новый поток. АБС возвращает данные о счетах клиента, сервис распознавания - извлечённый текст из документов.       | Response | HTTPS + mTLS       | ~100 КБ                       |
| П13 | Запрос внешних данных          | Слой: Интеграция (AI Gateway / MCP)                            | Слой: Внешние API (ФНС/ЕГРЮЛ/БКИ)                              | HTTPS POST: {inn, ogrn, query_type: "entity_verification"}                                                        | Новый поток. MCP Proxy запрашивает данные из внешних реестров (ФНС, ЕГРЮЛ, БКИ) для верификации клиента.           | Request  | HTTPS + mTLS       | ~30 КБ                        |
| П14 | Данные от внешних API          | Слой: Внешние API (ФНС/ЕГРЮЛ/БКИ)                              | Слой: Интеграция (AI Gateway / MCP)                            | {entity_data: {...}, credit_score: 750, registration_status: "active"}                                            | Новый поток. Внешние API возвращают данные о контрагенте, кредитную историю и статус регистрации.                  | Response | HTTPS + mTLS       | ~50 КБ                        |
| П15 | Запрос LLM/документов          | Слой: Интеграция (AI Gateway / MCP)                            | Слой: Облако (LLM, S3)                                         | OpenAI-compatible: {model: "gigachat-max-2", messages: \[...\]} + S3 GET: {bucket: "credit-apps", key: "doc.pdf"} | Новый поток. LiteLLM проксирует запрос к LLM для инференса, S3 - загрузка/выгрузка документов.                     | Request  | HTTPS + mTLS       | ~50 КБ (LLM) + до 200 МБ (S3) |
| П16 | Ответ LLM/файлы                | Слой: Облако (LLM, S3)                                         | Слой: Интеграция (AI Gateway / MCP)                            | {response: "Одобрить кредит", confidence: 0.95} + S3 file: {file_uri: "s3://..."}                                 | Новый поток. LLM возвращает решение по скорингу, S3 - обработанные файлы.                                          | Response | HTTPS + mTLS       | ~50 КБ (LLM) + до 200 МБ (S3) |
| П17 | Ответ от систем                | Слой: Интеграция (AI Gateway / MCP)                            | Слой: Агенты (Review, Scoring, Document, Guardian, Compliance) | {tool_result: {...}, status: "success", data: {...}}                                                              | Новый поток. MCP Proxy возвращает агентам агрегированные результаты от корпоративных и внешних систем.             | Response | MCP (HTTPS)        | ~100 КБ                       |
| П18 | Результат обработки            | Слой: Агенты (Review, Scoring, Document, Guardian, Compliance) | Слой: Оркестратор (Temporal, A2A/Kafka)                        | A2A: {task_id, result: {...}, evidence_bundle: {...}, eval_results: {...}}                                        | Новый поток. Агенты возвращают Orchestrator результаты обработки с Evidence Bundle.                                | Response | A2A (JSON-RPC)     | ~100 КБ                       |
| П19 | Результат заявки               | Слой: Оркестратор (Temporal, A2A/Kafka)                        | Слой: Интерфейс (IDP UI)                                       | JSON: {application_id, decision: "approve/reject", risk_score, explanation}                                       | Новый поток. Orchestrator возвращает финальное решение по заявке в IDP UI.                                         | Response | HTTPS              | ~10 КБ                        |
| П20 | Решение по заявке              | Слой: Интерфейс (IDP UI)                                       | Кредитный менеджер                                             | JSON: {application_id, decision, risk_score, explanation, next_steps} + файлы                                     | Новый поток. IDP Portal визуализирует решение с объяснением для менеджера.                                         | Response | HTTPS              | ~10 КБ                        |
